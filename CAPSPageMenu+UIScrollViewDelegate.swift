    ...
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if !didLayoutSubviewsAfterRotation {
            if scrollView.isEqual(controllerScrollView) {
                print("scrollView.contextOffet.x : \(scrollView.contentOffset.x) / page width : \(self.view.frame.width)")
                
                if scrollView.contentOffset.x >= 0.0 && scrollView.contentOffset.x <= (CGFloat(controllerArray.count - 1) * self.view.frame.width) {
                    print("currentOrientationIsPortrait : \(currentOrientationIsPortrait)")
                    print("phone : \(UIApplication.shared.statusBarOrientation.isPortrait)")
                    if (currentOrientationIsPortrait && orientationIsPortrait()) || (!currentOrientationIsPortrait && !orientationIsPortrait()) {
